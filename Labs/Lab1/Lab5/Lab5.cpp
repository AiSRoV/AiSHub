...
int main() {
    ...
    if (train.is_open()) {
        train << std::endl << "Всего выживших: " << surv << "   Выживших мужчин " << male << "   Выживших женщин " << female << std::endl;
        train << "Первый класс " << first << "   второй класс " << second << "    третий класс " << third << std::endl;
        train << "Средний возраст " << age / (all_m + all_w) << "   Ср.в.м " << m_age / all_m << "   Ср.в.ж " << w_age / all_w << std::endl;
        if (q == std::max(q, std::max(s, c))) train << "Q";
        else if (c == std::max(q, std::max(s, c))) train << "C";
        else if (s == std::max(q, std::max(s, c))) train << "S";
    }
    train.close();
